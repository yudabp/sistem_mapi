    ...
    <div class="bg-white dark:bg-gray-800 rounded-sm border border-gray-200 dark:border-gray-700 shadow-sm mb-8">
        <header class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60">
            <h2 class="font-semibold text-gray-800 dark:text-gray-100">
                {{ $editing_id ? 'Edit Nomor Kendaraan' : 'Tambah Nomor Kendaraan' }}
            </h2>
        </header>
        <div class="p-6 space-y-6">
            <form wire:submit.prevent="saveVehicleNumber" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Number -->
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    >
                        {{ $editing_id ? 'Perbarui' : 'Simpan' }} Nomor Kendaraan
                    </button>
                ...
                <table class="table-auto w-full">
                    <thead>
                        <tr class="text-xs font-semibold uppercase text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-700/30">
                            <th class="p-2 whitespace-nowrap">No Polisi</th>
                        ...
                        @forelse($vehicle_numbers as $vehicle_number)
                            <tr>
                                <td class="p-2 whitespace-nowrap">
                                    <div class="text-left font-medium text-gray-800 dark:text-gray-100">{{ $vehicle_number->number }}</div>
                                </td>
                                <td class="p-2 whitespace-nowrap">
                                    <div class="text-left">{{ $vehicle_number->description ?? '-' }}</div>
                                </td>
                                <td class="p-2 whitespace-nowrap">
                                ...
                                <td class="p-2 whitespace-nowrap">
                                    <button 
                                        wire:click="editVehicleNumber({{ $vehicle_number->id }})"
                                        class="px-3 py-1 bg-amber-600 text-white rounded hover:bg-amber-700 text-sm mr-2"
                                    >
                                        Edit
                                    </button>
                                    <button 
                                        wire:click="deleteVehicleNumber({{ $vehicle_number->id }})"
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="p-2 text-center text-gray-500 dark:text-gray-400">
                                    Tidak ada data nomor kendaraan
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
