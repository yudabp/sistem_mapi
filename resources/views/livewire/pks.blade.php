<div>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="mb-8 flex justify-between items-center">
            <div>
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">PKS</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Master data Pabrik Kelapa Sawit (PKS)</p>
            </div>
            <button
        ...
        <div class="bg-white dark:bg-gray-800 rounded-sm border border-gray-200 dark:border-gray-700 shadow-sm mb-6 p-6">
            <div class="flex flex-col sm:flex-row gap-4">
                <div class="flex-1">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Cari PKS</label>
                    <input
                        type="text"
                        wire:model.live="search"
                        placeholder="Cari nama PKS atau deskripsi"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-sm border border-gray-200 dark:border-gray-700 shadow-sm">
            ...
            <div class="p-3">
                    ...
                    <table class="table-auto w-full">
                        <thead>
                            <tr class="text-xs font-semibold uppercase text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-700/30">
                                <th class="p-2 whitespace-nowrap text-left">Nama PKS</th>
                        ...
                        <tbody class="text-sm divide-y divide-gray-100 dark:divide-gray-700/60">
                            @forelse($pks_list as $pk)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                    <td class="p-2 whitespace-nowrap">
                                        <div class="font-medium text-gray-800 dark:text-gray-100">{{ $pk->name }}</div>
                                    </td>
                                    <td class="p-2 whitespace-nowrap">
                                        <div class="text-gray-600 dark:text-gray-400">{{ $pk->description ?? '-' }}</div>
                                    </td>
                                    <td class="p-2 whitespace-nowrap">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                            {{ $pk->is_active ? 'bg-green-100 text-green-800 dark:bg-green-800/30 dark:text-green-500' : 'bg-red-100 text-red-800 dark:bg-red-800/30 dark:text-red-500' }}">
                                            {{ $pk->is_active ? 'Aktif' : 'Tidak Aktif' }}
                                        </span>
                                    </td>
                                    <td class="p-2 whitespace-nowrap">
                                        <div class="flex justify-center gap-2">
                                            <button
                                                wire:click="openEditModal({{ $pk->id }})"
                                            ...
                                            <button
                                                wire:click="confirmDelete({{ $pk->id }})"
                                ...
                                <tr>
                                    <td colspan="4" class="p-8 text-center text-gray-500 dark:text-gray-400">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-lg font-medium">Tidak ada data PKS</p>
                                            <p class="text-sm mt-1">Silakan tambahkan data PKS terlebih dahulu</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                ...
                <!-- Pagination Links -->
                @if($pks_list && $pks_list->hasPages())
                    <div class="mt-4 px-3">
                        {{ $pks_list->links('pagination::livewire-tailwind') }}
                    </div>
                @endif
            </div>
        </div>
    ...
    <x-dialog-modal wire:model.live="showModal">
        <x-slot name="title">
            {{ $isEditing ? 'Edit PKS' : 'Tambah PKS' }}
        </x-slot>

        <x-slot name="content">
            <form wire:submit.prevent="savePks">
                @csrf
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Nama PKS <span class="text-red-500">*</span>
                        </label>
                        <input
                            wire:model="name"
                            type="text"
                    ...
                    <div class="flex items-center">
                        ...
                        />
                        <label for="pks_is_active" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">
                            Aktif
                        </label>
                    </div>
                </div>
        ...
        <x-slot name="footer">
            ...
            <x-primary-button class="ms-3" wire:click="savePks" wire:loading.attr="disabled">
    ...
    <x-confirmation-modal wire:model.live="showDeleteConfirmation">
        ...
        <x-slot name="content">
            Apakah Anda yakin ingin menghapus data PKS ini?
            <br><br>
            <strong>{{ $deletingPksName ?? '' }}</strong>
            <br><br>
        ...
        <x-slot name="footer">
            ...
            <x-danger-button class="ms-3" wire:click="deletePks" wire:loading.attr="disabled">
                Hapus
            </x-danger-button>
        </x-slot>
