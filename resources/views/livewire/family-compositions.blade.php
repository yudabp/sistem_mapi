    ...
    <div class="bg-white dark:bg-gray-800 rounded-sm border border-gray-200 dark:border-gray-700 shadow-sm mb-8">
        <header class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60">
            <h2 class="font-semibold text-gray-800 dark:text-gray-100">
                {{ $editing_id ? 'Edit Susunan Keluarga' : 'Tambah Susunan Keluarga' }}
            </h2>
        </header>
        <div class="p-6 space-y-6">
            <form wire:submit.prevent="saveFamilyComposition" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Number -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" for="number">
                        Jumlah Susunan Keluarga
                    </label>
                    <input 
                        id="number"
                        ...
                        type="number" 
                        wire:model="number"
                        placeholder="Masukkan jumlah susunan keluarga"
                    />
                    @error('number') 
                ...
                <div class="md:col-span-2">
                    ...
                    >
                        {{ $editing_id ? 'Perbarui' : 'Simpan' }} Susunan Keluarga
                    </button>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-sm border border-gray-200 dark:border-gray-700 shadow-sm mb-8 p-6">
        <div>
            ...
            <input 
                ...
                placeholder="Cari jumlah atau deskripsi"
                class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-gray-300"
            />
        </div>
    </div>
            ...
            <div class="overflow-x-auto">
                <table class="table-auto w-full">
                    ...
                    <tbody class="text-sm divide-y divide-gray-100 dark:divide-gray-700/60">
                        @forelse($family_compositions as $family_composition)
                            <tr>
                                <td class="p-2 whitespace-nowrap">
                                    <div class="text-left font-medium text-gray-800 dark:text-gray-100">{{ $family_composition->number }}</div>
                                </td>
                                <td class="p-2 whitespace-nowrap">
                                    <div class="text-left">{{ $family_composition->description ?? '-' }}</div>
                                </td>
                                <td class="p-2 whitespace-nowrap">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                        {{ $family_composition->is_active ? 'bg-green-100 text-green-800 dark:bg-green-800/30 dark:text-green-500' : 'bg-red-100 text-red-800 dark:bg-red-800/30 dark:text-red-500' }}">
                                        {{ $family_composition->is_active ? 'Aktif' : 'Tidak Aktif' }}
                                    </span>
                                </td>
                                <td class="p-2 whitespace-nowrap">
                                    <button 
                                        wire:click="editFamilyComposition({{ $family_composition->id }})"
                                        class="px-3 py-1 bg-amber-600 text-white rounded hover:bg-amber-700 text-sm mr-2"
                                    >
                                        Edit
                                    </button>
                                    <button 
                                        wire:click="deleteFamilyComposition({{ $family_composition->id }})"
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="p-2 text-center text-gray-500 dark:text-gray-400">
                                    Tidak ada data susunan keluarga
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
