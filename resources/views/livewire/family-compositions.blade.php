<div>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="mb-8 flex justify-between items-center">
            <div>
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Susunan Keluarga</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Master data susunan keluarga karyawan</p>
            </div>
            <button
        ...
        <div class="bg-white dark:bg-gray-800 rounded-sm border border-gray-200 dark:border-gray-700 shadow-sm mb-6 p-6">
            <div class="flex flex-col sm:flex-row gap-4">
                <div class="flex-1">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Cari Susunan Keluarga</label>
                    <input
                        type="text"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-sm border border-gray-200 dark:border-gray-700 shadow-sm">
            <div class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60 flex justify-between items-center">
                <div class="flex justify-between items-center w-full">
                    <h2 class="font-semibold text-gray-800 dark:text-gray-100">Daftar Susunan Keluarga</h2>
            ...
            <div class="p-3">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                    ...
                                    <td class="p-2 whitespace-nowrap">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                            {{ $familyComposition->is_active ? 'bg-green-100 text-green-800 dark:bg-green-800/30 dark:text-green-500' : 'bg-red-100 text-red-800 dark:bg-red-800/30 dark:text-red-500' }}">
                                            {{ $familyComposition->is_active ? 'Aktif' : 'Tidak Aktif' }}
                                        </span>
                                    </td>
                                    <td class="p-2 whitespace-nowrap">
                                        <div class="flex justify-center gap-2">
                                            <button
                                                wire:click="openEditModal({{ $familyComposition->id }})"
                                            ...
                                            <button
                                                wire:click="confirmDelete({{ $familyComposition->id }})"
                                ...
                                <tr>
                                    <td colspan="4" class="p-8 text-center text-gray-500 dark:text-gray-400">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-lg font-medium">Tidak ada data susunan keluarga</p>
                                            <p class="text-sm mt-1">Silakan tambahkan data susunan keluarga terlebih dahulu</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                ...
                <!-- Pagination Links -->
                @if($family_compositions && $family_compositions->hasPages())
                    <div class="mt-4 px-3">
                        {{ $family_compositions->links('pagination::livewire-tailwind') }}
                    </div>
                @endif
            </div>
        </div>
    ...
    <x-dialog-modal wire:model.live="showModal">
        <x-slot name="title">
            {{ $isEditing ? 'Edit Susunan Keluarga' : 'Tambah Susunan Keluarga' }}
        </x-slot>

        <x-slot name="content">
            <form wire:submit.prevent="saveFamilyComposition">
                @csrf
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center">
                        <input
                            ...
                            type="checkbox"
                            id="family_composition_is_active"
                            class="h-4 w-4 text-violet-600 focus:ring-violet-500 border-gray-300 rounded"
                        />
                        <label for="family_composition_is_active" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">
                            Aktif
                        </label>
                    </div>
                </div>
        ...
        <x-slot name="footer">
            ...
            <x-primary-button class="ms-3" wire:click="saveFamilyComposition" wire:loading.attr="disabled">
    ...
    <x-confirmation-modal wire:model.live="showDeleteConfirmation">
        <x-slot name="title">
            Hapus Susunan Keluarga
        </x-slot>

        <x-slot name="content">
            Apakah Anda yakin ingin menghapus data susunan keluarga ini?
            <br><br>
            <strong>{{ $deletingFamilyCompositionInfo ?? '' }}</strong>
            <br><br>
        ...
        <x-slot name="footer">
            ...
            <x-danger-button class="ms-3" wire:click="deleteFamilyComposition" wire:loading.attr="disabled">
                Hapus
            </x-danger-button>
        </x-slot>
