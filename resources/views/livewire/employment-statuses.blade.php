    ...
    <div class="bg-white dark:bg-gray-800 rounded-sm border border-gray-200 dark:border-gray-700 shadow-sm mb-8">
        <header class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60">
            <h2 class="font-semibold text-gray-800 dark:text-gray-100">
                {{ $editing_id ? 'Edit Status Karyawan' : 'Tambah Status Karyawan' }}
            </h2>
        </header>
        <div class="p-6 space-y-6">
            <form wire:submit.prevent="saveEmploymentStatus" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Name -->
                <div>
                ...
                <!-- Value -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" for="value">
                        Nilai Status
                    </label>
                    <input 
                        id="value"
                        ...
                        type="text" 
                        wire:model="value"
                        placeholder="Masukkan nilai status"
                    />
                    @error('value') 
                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span> 
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    >
                        {{ $editing_id ? 'Perbarui' : 'Simpan' }} Status Karyawan
                    </button>
                ...
                <table class="table-auto w-full">
                    <thead>
                        <tr class="text-xs font-semibold uppercase text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-700/30">
                            <th class="p-2 whitespace-nowrap">Nama Status</th>
                            <th class="p-2 whitespace-nowrap">Nilai Status</th>
                    ...
                    <tbody class="text-sm divide-y divide-gray-100 dark:divide-gray-700/60">
                        @forelse($employment_statuses as $employment_status)
                            <tr>
                                <td class="p-2 whitespace-nowrap">
                                    <div class="text-left font-medium text-gray-800 dark:text-gray-100">{{ $employment_status->name }}</div>
                                </td>
                                <td class="p-2 whitespace-nowrap">
                                    <div class="text-left">{{ $employment_status->value }}</div>
                                </td>
                                <td class="p-2 whitespace-nowrap">
                                    <div class="text-left">{{ $employment_status->description ?? '-' }}</div>
                                </td>
                                <td class="p-2 whitespace-nowrap">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                        {{ $employment_status->is_active ? 'bg-green-100 text-green-800 dark:bg-green-800/30 dark:text-green-500' : 'bg-red-100 text-red-800 dark:bg-red-800/30 dark:text-red-500' }}">
                                        {{ $employment_status->is_active ? 'Aktif' : 'Tidak Aktif' }}
                                    </span>
                                </td>
                                <td class="p-2 whitespace-nowrap">
                                    <button 
                                        wire:click="editEmploymentStatus({{ $employment_status->id }})"
                                        class="px-3 py-1 bg-amber-600 text-white rounded hover:bg-amber-700 text-sm mr-2"
                                    >
                                        Edit
                                    </button>
                                    <button 
                                        wire:click="deleteEmploymentStatus({{ $employment_status->id }})"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="p-2 text-center text-gray-500 dark:text-gray-400">
                                    Tidak ada data status karyawan
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
