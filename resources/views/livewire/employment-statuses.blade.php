<div>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="mb-8 flex justify-between items-center">
            <div>
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Status Karyawan</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Master data status karyawan perusahaan</p>
            </div>
            <button
        ...
        <div class="bg-white dark:bg-gray-800 rounded-sm border border-gray-200 dark:border-gray-700 shadow-sm mb-6 p-6">
            <div class="flex flex-col sm:flex-row gap-4">
                <div class="flex-1">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Cari Status Karyawan</label>
                    <input
                        type="text"
                        wire:model.live="search"
                        placeholder="Cari nama, nilai, atau deskripsi status karyawan"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-sm border border-gray-200 dark:border-gray-700 shadow-sm">
            <div class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60 flex justify-between items-center">
                <div class="flex justify-between items-center w-full">
                    <h2 class="font-semibold text-gray-800 dark:text-gray-100">Daftar Status Karyawan</h2>
            ...
            <div class="p-3">
                    ...
                    <table class="table-auto w-full">
                        <thead>
                            <tr class="text-xs font-semibold uppercase text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-700/30">
                                <th class="p-2 whitespace-nowrap text-left">Nama Status</th>
                                <!-- <th class="p-2 whitespace-nowrap text-left">Nilai</th> -->
                        ...
                        <tbody class="text-sm divide-y divide-gray-100 dark:divide-gray-700/60">
                            @forelse($employment_statuses as $employmentStatus)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                    <td class="p-2 whitespace-nowrap">
                                        <div class="font-medium text-gray-800 dark:text-gray-100">{{ $employmentStatus->name }}</div>
                                    </td>
                                    <!-- <td class="p-2 whitespace-nowrap">
                                        <div class="text-gray-600 dark:text-gray-400">{{ $employmentStatus->value }}</div>
                                    </td> -->
                                    <td class="p-2 whitespace-nowrap">
                                        <div class="text-gray-600 dark:text-gray-400">{{ $employmentStatus->description ?? '-' }}</div>
                                    </td>
                                    <td class="p-2 whitespace-nowrap">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                            {{ $employmentStatus->is_active ? 'bg-green-100 text-green-800 dark:bg-green-800/30 dark:text-green-500' : 'bg-red-100 text-red-800 dark:bg-red-800/30 dark:text-red-500' }}">
                                            {{ $employmentStatus->is_active ? 'Aktif' : 'Tidak Aktif' }}
                                        </span>
                                    </td>
                                    <td class="p-2 whitespace-nowrap">
                                        <div class="flex justify-center gap-2">
                                            <button
                                                wire:click="openEditModal({{ $employmentStatus->id }})"
                                            ...
                                            <button
                                                wire:click="confirmDelete({{ $employmentStatus->id }})"
                                ...
                                <tr>
                                    <td colspan="5" class="p-8 text-center text-gray-500 dark:text-gray-400">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-lg font-medium">Tidak ada data status karyawan</p>
                                            <p class="text-sm mt-1">Silakan tambahkan data status karyawan terlebih dahulu</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                ...
                <!-- Pagination Links -->
                @if($employment_statuses && $employment_statuses->hasPages())
                    <div class="mt-4 px-3">
                        {{ $employment_statuses->links('pagination::livewire-tailwind') }}
                    </div>
                @endif
            </div>
        </div>
    ...
    <x-dialog-modal wire:model.live="showModal">
        <x-slot name="title">
            {{ $isEditing ? 'Edit Status Karyawan' : 'Tambah Status Karyawan' }}
        </x-slot>

        <x-slot name="content">
            <form wire:submit.prevent="saveEmploymentStatus">
                @csrf
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            required
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-gray-300"
                            placeholder="Masukkan nama status karyawan"
                        />
                        @error('name')
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Nilai Status <span class="text-red-500">*</span>
                        </label>
                        <input
                            wire:model="value"
                            type="text"
                            required
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-gray-300"
                            placeholder="Masukkan nilai status karyawan"
                        />
                        @error('value')
                            <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="flex items-center">
                        <input
                            ...
                            id="employment_status_is_active"
                            class="h-4 w-4 text-violet-600 focus:ring-violet-500 border-gray-300 rounded"
                        />
                        <label for="employment_status_is_active" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">
                            Aktif
                        </label>
                    </div>
                </div>
        ...
        <x-slot name="footer">
            ...
            <x-primary-button class="ms-3" wire:click="saveEmploymentStatus" wire:loading.attr="disabled">
    ...
    <x-confirmation-modal wire:model.live="showDeleteConfirmation">
        ...
        <x-slot name="content">
            Apakah Anda yakin ingin menghapus data status karyawan ini?
            <br><br>
            <strong>{{ $deletingEmploymentStatusName ?? '' }}</strong>
            <br><br>
        ...
        <x-slot name="footer">
            ...
            <x-danger-button class="ms-3" wire:click="deleteEmploymentStatus" wire:loading.attr="disabled">
                Hapus
            </x-danger-button>
        </x-slot>
