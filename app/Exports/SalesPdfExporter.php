...
class SalesPdfExporter
{
    ...
    public function generate()
    {
        ...
        $html .= '<title>Sales Data Export</title>';
        $html .= '<style>';
        ...
        $html .= '<div>Laporan Data Penjualan</div>';
        $html .= '</div>';
        $html .= '</div>';

        $html .= '<div class="document-title">';
        $html .= 'Sales Data Export';
        $html .= '</div>';
        ...
        $html .= '<th>TBS Quantity</th>';
        $html .= '<th>KG Quantity</th>';
        $html .= '<th>Price per KG</th>';
        ...
        foreach ($sales as $sale) {
            $html .= '<tr>';
            $html .= '<td>' . htmlspecialchars($sale->id) . '</td>';
            $html .= '<td>' . htmlspecialchars($sale->sp_number) . '</td>';
            $html .= '<td class="text-right">' . number_format($sale->tbs_quantity, 2, ',', '.') . '</td>';
            $html .= '<td class="text-right">' . number_format($sale->kg_quantity, 2, ',', '.') . '</td>';
            $html .= '<td class="text-right">' . number_format($sale->price_per_kg, 2, ',', '.') . '</td>';
            $html .= '<td class="text-right">' . number_format($sale->total_amount, 2, ',', '.') . '</td>';
            $html .= '<td>' . htmlspecialchars($sale->sale_date->format('d-m-Y')) . '</td>';
            $html .= '<td>' . e($sale->customer_name) . '</td>';
            $html .= '<td>' . e($sale->customer_address) . '</td>';
            $html .= '<td>' . htmlspecialchars($sale->created_at->format('d-m-Y H:i:s')) . '</td>';
            $html .= '<td>' . htmlspecialchars($sale->updated_at->format('d-m-Y H:i:s')) . '</td>';
            $html .= '</tr>';
            
            // Add to totals
            $totalTbsQuantity += $sale->tbs_quantity;
            $totalKgQuantity += $sale->kg_quantity;
        ...
        $html .= '<td></td>'; // Empty cell for price per kg
        ...
        $html .= '<p>Total Records: ' . count($sales) . '</p>';
