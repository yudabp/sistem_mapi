...
class FinancialPdfExporter
{
    ...
    public function generate()
    {
        ...
        $html .= '<title>Financial Data Export</title>';
        $html .= '<style>';
        ...
        $html .= '<div>Laporan Data Keuangan</div>';
        $html .= '</div>';
        $html .= '</div>';

        $html .= '<div class="document-title">';
        $html .= 'Financial Data Export';
        $html .= '</div>';
        ...
        foreach ($transactions as $transaction) {
            ...
            $html .= '<td>' . htmlspecialchars($transaction->transaction_number) . '</td>';
            $html .= '<td>' . htmlspecialchars($transaction->transaction_date->format('Y-m-d')) . '</td>';
            $html .= '<td>' . htmlspecialchars(ucfirst($transaction->transaction_type)) . '</td>';
            $html .= '<td class="text-right">' . number_format($transaction->amount, 2) . '</td>';
            $html .= '<td>' . htmlspecialchars($transaction->source_destination) . '</td>';
            $html .= '<td>' . htmlspecialchars($transaction->received_by) . '</td>';
            $html .= '<td>' . htmlspecialchars($transaction->notes) . '</td>';
            $html .= '<td>' . htmlspecialchars($transaction->category) . '</td>';
            $html .= '<td>' . htmlspecialchars($transaction->created_at->format('Y-m-d H:i:s')) . '</td>';
            $html .= '<td>' . htmlspecialchars($transaction->updated_at->format('Y-m-d H:i:s')) . '</td>';
            $html .= '</tr>';
        ...
        $html .= '<p>Total Records: ' . count($transactions) . '</p>';
