...
class ProductionPdfExporter
{
    ...
    public function generate()
    {
        ...
        $html .= '<title>Production Data Export</title>';
        $html .= '<style>';
        ...
        $html .= '<div>Laporan Data Produksi</div>';
        $html .= '</div>';
        $html .= '</div>';

        $html .= '<div class="document-title">';
        $html .= 'Production Data Export';
        $html .= '</div>';
        ...
        $html .= '<th>Date</th>';
        $html .= '<th>SP Number</th>';
        $html .= '<th>Vehicle</th>';
        $html .= '<th>TBS Qty (KG)</th>';
        $html .= '<th>KG Qty</th>';
        $html .= '<th>Division</th>';
        $html .= '<th>PKS</th>';
        ...
        foreach ($productions as $production) {
            $html .= '<tr>';
            $html .= '<td>' . htmlspecialchars($production->id) . '</td>';
            $html .= '<td>' . htmlspecialchars($production->transaction_number) . '</td>';
            $html .= '<td>' . htmlspecialchars($production->date->format('Y-m-d')) . '</td>';
            $html .= '<td>' . htmlspecialchars($production->sp_number) . '</td>';
            $html .= '<td>' . htmlspecialchars($production->vehicle_number) . '</td>';
            $html .= '<td class="text-right">' . number_format($production->tbs_quantity, 2) . '</td>';
            $html .= '<td class="text-right">' . number_format($production->kg_quantity, 2) . '</td>';
            $html .= '<td>' . htmlspecialchars($production->division) . '</td>';
            $html .= '<td>' . htmlspecialchars($production->pks) . '</td>';
            $html .= '<td>' . htmlspecialchars($production->created_at->format('Y-m-d H:i:s')) . '</td>';
            $html .= '<td>' . htmlspecialchars($production->updated_at->format('Y-m-d H:i:s')) . '</td>';
            $html .= '</tr>';
            
            // Add to totals
            $totalTbsQuantity += $production->tbs_quantity;
            $totalKgQuantity += $production->kg_quantity;
        }

        // Add total row
        $html .= '<tr class="total-row">';
        $html .= '<td colspan="5"><strong>TOTAL</strong></td>';
        $html .= '<td class="text-right"><strong>' . number_format($totalTbsQuantity, 2) . '</strong></td>';
        $html .= '<td class="text-right"><strong>' . number_format($totalKgQuantity, 2) . '</strong></td>';
        $html .= '<td colspan="4"></td>'; // Empty cells for the remaining columns
        $html .= '</tr>';
        ...
        $html .= '<p>Total Records: ' . count($productions) . '</p>';
